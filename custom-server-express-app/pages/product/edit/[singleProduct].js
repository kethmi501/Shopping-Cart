...
const EditProduct = () => {
    ...
    useEffect(() => {
        axios.get('/api/fetch-product', {
            params: {
                productId: router.query.singleProduct
            }
        }).then(res => {
            setName(res.data.product.name);
            setPrice(res.data.product.price);
            setImage({
                name: res.data.product.picture.name,
                url: res.data.product.picture.url
            });
    ...
    const editProduct = () => {
        if (name === '' || price === '' || category === '' || stock === '' || image.url === '') {
            ...
            axios.put('/api/update-product', {
                _id: router.query.singleProduct,
                name,
                price,
                category,
                stock,
        ...
        <div>
            <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md ">
                <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
                    <p className={`text-2xl font-semibold pb-2 text-indigo-600`}>Edit Product</p>
